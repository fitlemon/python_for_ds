### <img src='../static/img/mipt-icon.png' width="70" height="30"> Курс "Программирование на Python" (Магистратура "Науки о данных" МФТИ - 2023/2024) 
---
# Конспект по Модулю #4 **Numpy** 


## Оглавление конспекта
1. [Numpy. Типы данных](numpy_basic.ipynb) 
2. [Numpy. Массивы ](numpy_arrays.ipynb) 
3. [Numpy. Операция с массивами](numpy_array_methods.ipynb)  
4. [Numpy. Операция с векторами](numpy_vectors.ipynb)
5. [Numpy. Случайные числа](numpy_random.ipynb)
6. [Numpy. Задачки](numpy_tests.ipynb)

---

### Дополнительня информация по модулю:
1. [Задачки с семинаров по Zoom](./seminars/)
2. [Документация по Numpy](https://numpy.org/doc/stable/reference/routines.statistics.html)